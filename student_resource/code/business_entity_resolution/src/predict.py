...
def run(repo_root: Path, k: int, score_method: str, stack_model: str, laya_batch_size: int,
    ...
    candidate_map = blocking.generate_candidates(s1_df, s2_df, s3_df, k=k, score_method=score_method)
    del normalized, s2_df, s3_df
    ...
    del candidate_map
    table = features.load_entity_table(repo_root, "test")
    ...
    if threshold_override is not None:
        ...
        with open(models_path / "ensemble_threshold.json") as f:
            threshold = json.load(f)[stack_model]
